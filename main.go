...
//
// Returns:
//   - GetCellRichTextResult structure containing:
...
func GetCellRichText(idx int, sheet, cell *C.char) C.struct_GetCellRichTextResult {
	...
	return C.struct_GetCellRichTextResult{
...
func SetDocProps(idx int, props *C.struct_DocProperties) *C.char {
	...
	goStructVal := reflect.ValueOf(props)
	cStructPtr := reflect.ValueOf(docProps)
	...
	convertedProps := v.Interface().(*excelize.DocProperties)

	err = f.(*excelize.File).SetDocProps(convertedProps)
	if err != nil {
		return C.CString(err.Error())
	}
	return nil
