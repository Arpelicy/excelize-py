...
class TestExcelize(unittest.TestCase):
    ...
    def test_cell_rich_text(self):
        ...
        self.assertIsNone(Err)
        self.assertEqual(len(Runs), len(rich_text_runs))
        
        for i, run in enumerate(rich_text_runs):
            self.assertEqual(Runs[i].text, run.text)
            if hasattr(run.font, 'bold'):
                self.assertEqual(Runs[i].font.bold, run.font.bold)
            if hasattr(run.font, 'italic'):
                self.assertEqual(Runs[i].font.italic, run.font.italic)
            if hasattr(run.font, 'color'):
                self.assertEqual(Runs[i].font.color.lower(), run.font.color.lower())
            if hasattr(run.font, 'family'):
                self.assertEqual(Runs[i].font.family, run.font.family)
            if hasattr(run.font, 'size'):
                self.assertEqual(Runs[i].font.size, run.font.size)
            if hasattr(run.font, 'strike'):
                self.assertEqual(Runs[i].font.strike, run.font.strike)
            if hasattr(run.font, 'vert_align'):
                if run.font.vert_align and Runs[i].font.vert_align:
                    self.assertEqual(Runs[i].font.vert_align, run.font.vert_align)
