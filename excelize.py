...
class File:
    ...
    def get_cell_rich_text(
            ...
            """
            Get rich text content from a cell in the specified worksheet.
            ...
            Returns:
                Tuple[List[RichTextRun], Optional[Exception]]: 
                    - List[RichTextRun]: List of rich text runs containing text and formatting
                    - Optional[Exception]: None if successful, Exception with error message if failed
            """
    ...
    def set_doc_props(self, props: DocProperties) -> Optional[Exception]:
        """
        Set document properties of the Excel file.
    
        Args:
            props (DocProperties): Document properties object containing metadata:
                - category: Document category
                - content_status: Content status information
                - created: Creation timestamp
                - creator: The document creator/author
        ...
        """
        if props is None:
            return Exception("Document properties cannot be None")
                
        c_props = py_value_to_c(props, types_go._DocProperties())
        if c_props is None:
            return Exception("Failed to convert properties")
        lib.SetDocProps.restype = c_char_p
        err = lib.SetDocProps(self.file_index, byref(c_props)).decode(ENCODE)
        return None if not err else Exception(err)
